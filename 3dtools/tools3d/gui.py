...
ROOT_3DTOOLS = os.path.join(os.path.dirname(FILE), "..")
...
class Open_images_window(Toplevel):
    def __init__(self, master):
        ...
        self.header_text = Label(
            self,
            # text="OGrEE-Tools/3dtools - open images",
            text="OGrEE Tools",
            bg="black",
            fg="white",
            font=("Helvetica, 20"),
        ...
        self.select_rear_image_button = Button(
            self,
            text="Select REAR\nface image",
            command=self.select_rear_image,
            bg="white",
            fg="black",
            height=4,
            width=10,
        ...
        self.select_front_image_button = Button(
            self,
            text="Select FRONT\nface image",
            command=self.select_front_image,
            bg="white",
            fg="black",
            height=4,
            width=10,
        ...
        self.rear_image_label = Label(
            self,
            textvariable=self.rear_image_var,
            bg="white",
            fg="black",
            height=4,
        )
        ...
        self.front_image_label = Label(
            self,
            textvariable=self.front_image_var,
            bg="white",
            fg="black",
            height=4,
        )
        ...
        self.unselect_rear_image_button = Button(
            self,
            text="X",
            font=("Helvetica, 20"),
            command=self.unselect_rear_image,
            fg="red",
            height=1,
            width=1,
        )
        ...
        self.unselect_front_image_button = Button(
            self,
            text="X",
            font=("Helvetica, 20"),
            command=self.unselect_front_image,
            fg="red",
            height=1,
            width=1,
        )
        ...
        self.select_json_template_button = Button(
            self,
            text="Select JSON\ntemplate",
            command=self.select_json_template,
            bg="white",
            fg="black",
            height=4,
            width=10,
        ...
        self.server_height_label = Label(
            self, text="Server height (mm):", bg="white", fg="black", height=4
        )
        self.server_height_label.grid(column=1, row=5)

        self.server_height_var = StringVar(self)
        self.server_height_entry = Entry(
            self,
            textvariable=self.server_height_var,
            bg="white",
            fg="black",
            width=14,
        )
        ...
        self.server_width_label = Label(
            self, text="Server width (mm):", bg="white", fg="black", height=4
        )
        self.server_width_label.grid(column=3, row=5)

        self.server_width_var = StringVar(self)
        self.server_width_entry = Entry(
            self,
            textvariable=self.server_width_var,
            bg="white",
            fg="black",
            width=14,
        )
    ...
    def select_json_template(self):
        ...
        if file:
            ...
            if (
                "sizeWDHmm" in data
                and isinstance(data["sizeWDHmm"], list)
                and len(data["sizeWDHmm"]) == 3
            ):
                ...
                messagebox.showwarning(
                    "Invalid JSON template", "Template file format is not valid"
                )
    ...
    def done(self):
        if self.rear_image_file == "" and self.front_image_file == "":
            ...
            self.server_height_entry.get() == ""
            or not self.server_height_entry.get().replace(".", "").isnumeric()
        ):
            messagebox.showwarning(
                "Invalid server height",
                "Server height should be a float.\n\nPlease, enter a valid server height before continuing.",
            )
        elif (
            self.server_width_entry.get() == ""
            or not self.server_width_entry.get().replace(".", "").isnumeric()
        ):
            messagebox.showwarning(
                "Invalid server width",
                "Server width should be a float.\n\nPlease, enter a valid server width before continuing.",
            )
        else:
            if self.front_image_file == "":
                ok = self.master.open_images(
                    {"rear": self.rear_image_file},
                    self.server_height_entry.get(),
                    self.server_width_entry.get(),
                    self.server_depth,
                    self.json_template_file,
                ...
                ok = self.master.open_images(
                    {"rear": self.rear_image_file, "front": self.front_image_file},
                    self.server_height_entry.get(),
                    self.server_width_entry.get(),
                    self.server_depth,
                    self.json_template_file,
...
class Gui(Tk):
    ...
    def open_images(self, images, height, width, depth, json_file):
        ...
        if len(images) == 1:
            try:
                ...
                os.remove(self.options["path640"])
            except FileNotFoundError:
                pass
            if len(self.classifiers) == 2:
                try:
                    os.remove(self.options["path_2"])
                    os.remove(self.options["path640_2"])
                except FileNotFoundError:
                    pass
            try:
                print("\n" + 94 * "=" + "\n")
                ...
                self.options["path"] = os.path.join(
                    ROOT_3DTOOLS, "api", "tmp" + file.split("/")[-1]
                )
                im = Image.open(file)
                w, h = im.size
                ...
                imsave(
                    ...
                    np.asarray(
                        im.resize(
                            (
                                int(tools.RATIO * self.options["width"]),
                                int(tools.RATIO * self.options["height"]),
                            )
                        )
                    ),
                )
                self.options["path640"] = os.path.join(
                    ROOT_3DTOOLS, "api", "s-tmp" + file.split("/")[-1]
                )
                imsave(
                    self.options["path640"],
                    np.asarray(Image.open(file).resize((640, 640))),
                )
                ...
                self.classifiers = [
                    Classifiers(file, float(height), float(width), face)
                ]
                ...
                messagebox.showwarning(
                    "File not found", f"The file '{file}' could not be found."
                )
                return False
        else:
            ok = self.open_images(
            ...
            if ok:
                try:
                    ...
                    self.options["path_2"] = os.path.join(
                        ROOT_3DTOOLS, "api", "tmp" + file.split("/")[-1]
                    )
                    imsave(
                        self.options["path_2"],
                    ...
                    self.options["path640_2"] = os.path.join(
                        ROOT_3DTOOLS, "api", "s-tmp" + file.split("/")[-1]
                    )
                    imsave(
                        self.options["path640_2"],
                        np.asarray(Image.open(file).resize((640, 640))),
                    )
                    ...
                    self.classifiers.append(
                        Classifiers(file, float(height), float(width), "front")
                    )
    ...
    def calculate_hitboxes(self):
        ...
        for i in range(len(self.classifiers)):
            ...
            for k in self.classifiers[i].components:
                name, compotype, _, angle, _, dimensions = self.classifiers[
                    i
                ].components[k]
                composhape = (
                    sizetable[compotype] if angle == 0 else sizetable[compotype][::-1]
                )
                pt1 = (int(k[0]), int(k[1]))
                if compotype == "PSU":
                    h, w = dimensions
                    ...
                    pt2 = (
                        int(pt1[0] + composhape[2] * tools.RATIO),
                        int(pt1[1] + composhape[0] * tools.RATIO),
                    )
    ...
    def detect_all(self):
        def detect(classifier, face, path640, path, servername):
            ...
            if classifier.components != {}:
                print(
                    "  - Some components have already been detected. Try detecting each component individually."
                )
            else:
                ...
                pred = self.model.predict(
                    ...
                    agnostic_nms=self.options["agnostic_nms"],
                    classes=self.options["classes"],
                    show=self.options["show"],
                    save=self.options["save"],
                    save_txt=self.options["save_txt"],
                    save_conf=self.options["save_conf"],
                    save_crop=self.options["save_crop"],
                    show_labels=self.options["show_labels"],
                    show_conf=self.options["show_conf"],
    ...
    def detect_slot(self):
        def detect(classifier, face, path640, path, servername):
            ...
            if any(
                [
                    compotype == "Slot_normal" or compotype == "Slot_lp"
                    for (_, compotype, *_) in classifier.components.values()
                ]
            ):
                print("  - Slots have already been detected.")
            else:
                pred = self.model.predict(
                    path640,
                    conf=self.options["conf"],
    ...
    def detect_disk(self):
        def detect(classifier, face, path640, path, servername):
            ...
            if any(
                [
                    compotype == "Disk_lff" or compotype == "Disk_sff"
                    for (_, compotype, *_) in classifier.components.values()
                ]
            ):
                print("  - Disks have already been detected.")
            else:
    ...
    def detect_psu(self):
        def detect(classifier, face, path640, path, servername):
            ...
            if any(
                ...
                print("  - PSUs have already been detected.")
            else:
    ...
    def detect_serial(self):
        def detect(classifier, face, path640, path, servername):
            ...
            if any(
                ...
                print("  - Serial ports have already been detected.")
            else:
    ...
    def detect_vga(self):
        def detect(classifier, face, path640, path, servername):
            ...
            if any(
                ...
                print("  - VGA ports have already been detected.")
            else:
    ...
    def reset(self, event=None):
        ...
        tools.drawcomponents_gui(
            self.options["servername"],
            self.options["path"],
            self.classifiers[0].components,
        )
        if len(self.classifiers) == 2:
            tools.drawcomponents_gui(
                self.options["servername_2"],
                self.options["path_2"],
                self.classifiers[1].components,
            )
        self.update_images()
    ...
    def save_component(self):
        if self.component_name_var.get() == "":
            ...
            self.component_depth_var.get() == ""
            or not self.component_depth_var.get().replace(".", "").isnumeric()
        ):
            print("\n" + 94 * "=" + "\n")
            ...
            if self.click_pt1 in self.classifiers[0].components:
                ...
                component = (
                    self.component_name_var.get(),
                    self.component_type_var.get(),
                    self.component_category_var.get(),
                    prev_component[3],
                    prev_component[4],
                    prev_component[5],
                )
                ...
                new_size = [
                    size[0],
                    round(float(self.component_depth_var.get()), 1),
                    size[2],
                ]
                ...
                print("\n" + 94 * "=" + "\n")
                print(
                    f"Saved changes to component {component[0]} of type {component[1]}."
                )
            elif self.selected_component in self.classifiers[0].components:
                prev_component = self.classifiers[0].components.pop(
                    self.selected_component
                )
                if (
                    self.click_pt2[1] - self.click_pt1[1]
                    < self.click_pt2[0] - self.click_pt1[0]
                ):
                    angle = 90
                else:
                    angle = 0
                ...
                size = [
                    round((self.click_pt2[1] - self.click_pt1[1]) / tools.RATIO, 1),
                    round(float(self.component_depth_var.get()), 1),
                    round((self.click_pt2[0] - self.click_pt1[0]) / tools.RATIO, 1),
                ]
                if angle != 0:
                    size = size[::-1]
                for classifier in self.classifiers:
            ...
            if self.click_pt1 in self.classifiers[1].components:
                ...
                prev_component = self.classifiers[1].components.pop(
                    self.selected_component
                )
                if (
                    self.click_pt2[1] - self.click_pt1[1]
    ...
    def follow(self, event, name):
        ...
        if name == "top":
            ...
            tools.draw_tmp_rectangle(
                self.options["path"],
                (self.click_pt1[1], self.click_pt1[0]),
                (pt2[1], pt2[0]),
                (147, 69, 52),
                self.component_type_var.get(),
            )
    ...
    def click(self, event, name):
        self.click_pt1 = (
            int(event.y / self.image_ratio),
            int(event.x / self.image_ratio),
        )

        if name == "top":
            self.image_labels[0].bind(
                "<Motion>", lambda event: self.follow(event, name)
            )
        else:
            self.image_labels[1].bind(
                "<Motion>", lambda event: self.follow(event, name)
            )
        ...
        if name == "top":
            ...
            if self.click_pt1 == self.click_pt2:
                for _, v in self.hitboxes[0].items():
                    if (
                        self.click_pt1[0] >= v["pt1"][0]
                        and self.click_pt1[0] <= v["pt2"][0]
                        and self.click_pt1[1] >= v["pt1"][1]
                        and self.click_pt1[1] <= v["pt2"][1]
                    ):
                        self.click_pt1 = v["pt1"]
                        self.selected_component = self.click_pt1
                        self.component_name_var.set(
                            self.classifiers[0].components[self.click_pt1][0]
                        )
                        self.component_type_var.set(
                            self.classifiers[0].components[self.click_pt1][1]
                        )
                        self.component_category_var.set(
                            self.classifiers[0].components[self.click_pt1][2]
                        )
                        self.component_depth_var.set(
                            self.classifiers[0].sizetable[
                                self.classifiers[0].components[self.click_pt1][1]
                            ][1]
                        )
                    ...
                    if self.component_type_var.get() in tools.SIZETABLE:
                        composhape = tools.SIZETABLE[self.component_type_var.get()]
                        self.click_pt2 = (
                            int(self.click_pt1[0] + composhape[2] * tools.RATIO),
                            int(self.click_pt1[1] + composhape[0] * tools.RATIO),
                        )
                        tools.drawcomponents_gui(
                            self.options["servername"],
                            self.options["path"],
                        ...
                        tools.draw_tmp_rectangle(
                            self.options["path"],
                            (self.click_pt1[1], self.click_pt1[0]),
                            (self.click_pt2[1], self.click_pt2[0]),
                            (133, 0, 82),
                            self.component_type_var.get(),
                        )
                        self.update_images()
                ...
                if self.selected_component is None:
                    ...
                    tools.draw_tmp_rectangle(
                        ...
                        (self.click_pt2[1], self.click_pt2[0]),
                        (133, 0, 82),
                        self.classifiers[0].components[self.selected_component][1],
                    )
                    self.update_images()
            ...
            if self.click_pt1 == self.click_pt2:
                for _, v in self.hitboxes[1].items():
                    if (
                        ...
                        self.selected_component = self.click_pt1
                        self.component_name_var.set(
                            self.classifiers[1].components[self.click_pt1][0]
                        )
                        self.component_type_var.set(
                            self.classifiers[1].components[self.click_pt1][1]
                        )
                        self.component_category_var.set(
                            self.classifiers[1].components[self.click_pt1][2]
                        )
                        self.component_depth_var.set(
                            self.classifiers[1].sizetable[
                                self.classifiers[1].components[self.click_pt1][1]
                            ][1]
                        )
                        self.component_selected()
                ...
                if self.selected_component is None:
                    ...
                    tools.draw_tmp_rectangle(
                        ...
                        (self.click_pt2[1], self.click_pt2[0]),
                        (133, 0, 82),
                        self.classifiers[1].components[self.selected_component][1],
                    )
                    self.update_images()
    ...
    def return_to_editing(self):
        ...
        for label in self.image_labels:
            label.bind(
                "<Button-1>",
                lambda event: self.click(event, str(event.widget).split(".")[-1]),
            )
            label.bind(
                "<ButtonRelease-1>",
                lambda event: self.release(event, str(event.widget).split(".")[-1]),
            )
        ...
        print("\n" + 94 * "=" + "\n")
        print(
            "Click 'Return to detection' if you wish to automatically detect another component."
        )
        print(
            "Click inside a rectangle on the image to select a component and edit its name, type and depth."
        )
        print(
            "Click on the top left corner after selecting a component to adjust its position."
        )
        print(
            "You can also draw a new rectangle to adjust its size. Press 'Esc' to unselect a component."
        )
        print(
            "You can create a new component by drawing its rectangle or by placing it (if type selected)."
        )
        print("Don't forget to enter a name, type and depth for the new component.")
        print(
            "Use the buttons to save your changes or to delete a component after selecting it."
        )
    ...
    def save_and_exit(self):
        ...
        if len(self.classifiers) == 2:
            if self.json_template_file == "":
                file_2 = self.options["servername_2"].split("/")[-1].split(".")[0] + (
                    "_front" if self.options["face_2"] == "front" else "_rear"
                )
                msg = f"{file_2}.json files saved in '3dtools/api/'\n\nThank you for using OGrEE-Tools/3dtools!"
            if messagebox.askokcancel("Files saved", msg):
                if self.json_template_file != "":
                    f = open(self.json_template_file)
                    data = json.load(f)
                    data["components"] = (
                        self.classifiers[0].getjson()["components"]
                        + self.classifiers[1].getjson()["components"]
                    )
                    data["slots"] = (
                        self.classifiers[0].getjson()["slots"]
                        + self.classifiers[1].getjson()["slots"]
                    )
            ...
            if self.json_template_file == "":
                file_1 = self.options["servername"].split("/")[-1].split(".")[0] + (
                    "_rear" if self.options["face"] == "rear" else "_front"
                )
                msg = f"{file_1}.json file saved in '3dtools/api/'\n\nThank you for using OGrEE-Tools/3dtools!"
    ...
    def create_json_window(self):
        ...
        self.return_to_editing_button = Button(
            self.tab_3dtools,
            text="Return to editing",
            command=self.return_to_editing,
            fg="black",
            height=3,
            width=16,
        )
        ...
        self.json_widgets = [
            self.return_to_editing_button,
            self.create_fbx_button,
    ...
    def create_editing_window(self):
        ...
        self.component_name_label = Label(
            self.tab_3dtools, text="Component name:", bg="white", fg="black", height=4
        )
        self.component_name_label.grid(column=0, row=3)

        self.component_name_entry = Entry(
            self.tab_3dtools,
            textvariable=self.component_name_var,
            bg="white",
            fg="black",
            width=14,
        )
        ...
        self.component_type_label = Label(
            self.tab_3dtools, text="Component type:", bg="white", fg="black", height=4
        )
        self.component_type_label.grid(column=0, row=4)

        self.values = list(tools.SIZETABLE.keys())
        self.component_type_entry = ttk.Combobox(
            self.tab_3dtools,
        ...
        self.component_category_label = Label(
            self.tab_3dtools,
        ...
        self.component_category_entry = ttk.Combobox(
            self.tab_3dtools,
            textvariable=self.component_category_var,
            values=self.values,
            background="white",
            foreground="black",
            width=13,
        ...
        self.component_category_entry.bind(
            "<<ComboboxSelected>>", self.component_selected
        )

        self.component_depth_label = Label(
            self.tab_3dtools,
            text="Component depth (mm):",
            bg="white",
            fg="black",
            height=4,
        )
        ...
        self.component_depth_entry = Entry(
            self.tab_3dtools,
            textvariable=self.component_depth_var,
            bg="white",
            fg="black",
            width=14,
        )
        ...
        self.component_wh_label_l = Label(
            self.tab_3dtools,
            text="Component width, height (mm):",
            bg="white",
            fg="black",
            height=4,
        )
        ...
        self.component_wh_label_r = Label(
            self.tab_3dtools,
            textvariable=self.component_wh_var,
            anchor="w",
            bg="white",
            fg="black",
            height=4,
        ...
        # Buttons on bottom leftmost columns
        self.save_component_button = Button(
            self.tab_3dtools,
            text="Save component",
        ...
        self.delete_component_button = Button(
            self.tab_3dtools,
        ...
        self.return_to_detection_button = Button(
            self.tab_3dtools,
            text="Return to\ndetection",
        ...
        self.finish_editing = Button(
            self.tab_3dtools,
            text="Finish editing",
            command=self.create_json_window,
            fg="black",
            height=4,
            width=16,
        )
        ...
        self.editing_widgets = [
            self.component_name_label,
            self.component_name_entry,
            self.component_type_label,
            self.component_type_entry,
            self.component_category_label,
            self.component_category_entry,
            self.component_depth_label,
            self.component_depth_entry,
            self.component_wh_label_l,
            self.component_wh_label_r,
            self.save_component_button,
    ...
    def load_square_tab(self):
        ...
        self.nsr_title.grid(columnspan=2, column=0, row=2)

        self.label_source = Label(
            self.tab_square,
            text="Path of the JSON room:",
            bg="white",
            font=("Helvetica, 12"),
        )
        self.label_source.grid(row=3, column=0, padx=10, pady=5)
        self.text_source = Text(
            self.tab_square, width=26, height=1, font=("Helvetica, 12")
        )
        self.text_source.grid(row=3, column=1, padx=10, pady=5)
        ...
        self.label_dest = Label(
            self.tab_square,
            text="Output file name (optional):",
            bg="white",
            font=("Helvetica, 12"),
        )
        self.label_dest.grid(row=4, column=0, padx=10, pady=5)
        self.text_dest = Text(
            self.tab_square, width=26, height=1, font=("Helvetica, 12")
        )
        self.text_dest.grid(row=4, column=1, padx=10, pady=5)

        self.label_angle = Label(
            self.tab_square,
            text="Tiling's angle (degree) (optional, 0 by default)",
            bg="white",
            font=("Helvetica, 12"),
        )
        self.label_angle.grid(row=5, column=0, padx=10, pady=5)
        self.text_angle = Text(
            self.tab_square, width=26, height=1, font=("Helvetica, 12")
        )
        self.text_angle.grid(row=5, column=1, padx=10, pady=5)

        self.label_offset = Label(
            self.tab_square,
            text="First tile's offset from the first vertex : x,y (m) (optional)",
            bg="white",
            font=("Helvetica, 12"),
        )
        self.label_offset.grid(row=6, column=0, padx=10, pady=5)
        self.text_offset = Text(
            self.tab_square, width=26, height=1, font=("Helvetica, 12")
        )
        self.text_offset.grid(row=6, column=1, padx=10, pady=5)

        self.label_tilesize = Label(
            self.tab_square,
            text="Size of a tile (m), (optional, 60cm by default)",
            bg="white",
            font=("Helvetica, 12"),
        )
        self.label_tilesize.grid(row=7, column=0, padx=10, pady=5)
        self.text_tilesize = Text(
            self.tab_square, width=26, height=1, font=("Helvetica, 12")
        )
        ...
        self.error_message = Label(
            self.tab_square,
            text="You must choose a path for the JSON room",
            bg="white",
            foreground="red",
            font=("Helvetica, 12"),
        )
        self.error_message.grid(row=9, column=0, padx=10, pady=5)
        self.error_message.grid_remove()

        self.draw_button = Button(
            self.tab_square,
            text="Draw",
            command=lambda: self.check_form(),
            fg="black",
            height=3,
            width=12,
            font=("Helvetica, 12"),
    ...
    def check_form(self):
        self.error_message.grid_remove()
        if not self.text_source.get("1.0", "end-1c"):
            self.error_message.grid()
        else:
            processJSON(
                path=self.text_source.get("1.0", "end-1c"),
                tileSize=self.text_tilesize.get("1.0", "end-1c")
                if self.text_tilesize.get("1.0", "end-1c")
                else None,
                angle=self.text_angle.get("1.0", "end-1c")
                if self.text_angle.get("1.0", "end-1c")
                else None,
                offset=self.text_offset.get("1.0", "end-1c")
                if self.text_offset.get("1.0", "end-1c")
                else None,
                draw=True,
    ...
    def load_tools3d_tab(self):
        ...
        self.open_images_label = Label(
            self.tab_3dtools,
            text="Welcome to OGrEE-Tools/3dtools!",
            bg="white",
            fg="black",
            font=("Helvetica, 16"),
            justify="center",
        ...
        self.open_images_label.grid(columnspan=2, column=0, row=2)

        self.open_images_button = Button(
            self.tab_3dtools,
            text="Open images\n(rear AND/OR front)",
            command=lambda: Open_images_window(self),
            fg="black",
            height=4,
            width=BigBtnWidth,
        )
        ...
        self.detect_all_button = Button(
            self.tab_3dtools,
            text="Detect all\ncomponents",
            command=self.detect_all,
            fg="black",
            height=4,
            width=14,
        )
        ...
        self.detect_slot_button = Button(
            self.tab_3dtools,
            text="Detect slots",
            command=self.detect_slot,
            fg="black",
            height=4,
            width=14,
        )
        ...
        self.detect_disk_button = Button(
            self.tab_3dtools,
            text="Detect disks",
            command=self.detect_disk,
            fg="black",
            height=4,
            width=14,
        )
        ...
        self.detect_psu_button = Button(
            self.tab_3dtools,
            text="Detect PSU",
            command=self.detect_psu,
            fg="black",
            height=4,
            width=14,
        )
        ...
        self.detect_serial_button = Button(
            self.tab_3dtools,
            text="Detect serial ports\n(DB9 connector)",
            command=self.detect_serial,
            fg="black",
            height=4,
            width=14,
        )
        ...
        self.detect_vga_button = Button(
            self.tab_3dtools,
            text="Detect VGA ports\n(DB15 connector)",
            command=self.detect_vga,
            fg="black",
            height=4,
            width=14,
        )
        ...
        self.detect_bmc_button = Button(
            self.tab_3dtools,
            text="Detect BMC",
            command=self.detect_bmc,
            fg="black",
            height=4,
            width=14,
        )
        ...
        self.detect_usb_button = Button(
            self.tab_3dtools,
            text="Detect USB ports",
        ...
        self.create_editing_window_button = Button(
            self.tab_3dtools,
            text="Finish detection",
            command=self.create_editing_window,
            fg="black",
            height=4,
            width=BigBtnWidth,
        )
        ...
        self.detection_widgets = [
            self.open_images_label,
            self.open_images_button,
            self.detect_all_button,
            self.detect_slot_button,
            self.detect_disk_button,
            self.detect_psu_button,
            self.detect_serial_button,
            self.detect_vga_button,
            self.detect_bmc_button,
            self.detect_usb_button,
        ...
        self.top_image_label = Label(self.tab_3dtools, name="top")
        self.top_image_label.grid(
            columnspan=3, rowspan=2, column=2, row=4, padx=(20, 20), pady=(20, 20)
        )

        self.bot_image_label = Label(self.tab_3dtools, name="bot")
        ...
        imsave(
            self.options["path"],
            np.asarray(
                Image.open(self.options["servername"]).resize(
                    (
                        int(tools.RATIO * self.options["width"]),
                        int(tools.RATIO * self.options["height"]),
                    )
        ...
        self.options["path640"] = os.path.join(
            ROOT_3DTOOLS, "api", "s-tmp" + self.options["servername"].split("/")[-1]
        )
        imsave(
            self.options["path640"],
            np.asarray(Image.open(self.options["servername"]).resize((640, 640))),
        )

        self.update_images()

        self.component_name_var = StringVar(self.tab_3dtools)
        self.component_type_var = StringVar(self.tab_3dtools)
        self.component_category_var = StringVar(self.tab_3dtools)
        self.component_depth_var = StringVar(self.tab_3dtools)
        self.component_wh_var = StringVar(self.tab_3dtools)
        self.click_pt1 = None
        self.selected_component = None
        self.hitboxes = [{}, {}]
        self.json_template_file = ""
    ...
    def create_detection_window(self):
        ...
        tabStyle.configure("TFrame", background="white")

        tabControl = ttk.Notebook(
            self, height=620, width=1280, padding=10, style="TNotebook"
        )

        self.tab_3dtools = ttk.Frame(tabControl, style="TFrame")
        self.tab_3dtools.grid(columnspan=5, rowspan=7, row=3, column=0)
    ...
    def __init__(self, options):
        ...
        self.options["path"] = os.path.join(
            ROOT_3DTOOLS, "api", "tmp" + self.options["servername"].split("/")[-1]
        )
