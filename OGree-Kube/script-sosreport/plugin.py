                ...
                if r['status'] == 0:
                    ...
                    for pod in pods:
                        ...
                        self.add_cmd_output('%s logs %s' % (k_cmd, pod),
                                            subdir='pods')
        if self.get_option('resume'):
            all_namespaces_command = "%s get deployments,pods,pvc,pv,storageclass --all-namespaces=true -o json" % self.kube_cmd
            
            # Add command output for resources from all namespaces
            self.add_cmd_output(all_namespaces_command, subdir='resume')
